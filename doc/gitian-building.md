...
*Setup instructions for a gitian build of Demo Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Demo Coin
...
- [Building Demo Coin](#building-democoin)
...
Clone the git repositories for democoin and gitian and then checkout the democoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the democoin repository
...
To build Demo Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the democoin repository.
...
```bash
    ...
    From https://github.com/democoin/democoin
...
For example:
```bash
URL=https://github.com/konez2k/democoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/democoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/democoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/democoin-build.assert
```
...
[democoin/gitian.sigs](https://github.com/democoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
